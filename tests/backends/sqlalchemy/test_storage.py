...
class TestCategoryManager():
    def test_add_new(self, alchemy_store, category):
        """
    ...
    def test_add_existing_name(self, alchemy_store, alchemy_category):
        """Make sure that adding a category with a name that is already present gives an error."""
        with pytest.raises(ValueError):
            alchemy_store.categories._add(alchemy_category.as_hamster())

    def test_add_with_pk(self, alchemy_store, alchemy_category):
    ...
    def test_update(self, alchemy_store, alchemy_category, new_category_values):
        ...
        updated_category = alchemy_store.session.query(AlchemyCategory).get(category.pk)
        count_after = alchemy_store.session.query(AlchemyCategory).count()
        assert count_before == count_after
        assert alchemy_category.as_hamster().equal_fields(updated_category)

    def test_update_without_pk(self, alchemy_store, alchemy_category):
        alchemy_category.pk = None
        with pytest.raises(ValueError):
            alchemy_store.categories._update(alchemy_category.as_hamster())
    ...
    def test_update_existing_name(self, alchemy_store, alchemy_category_factory):
        ...
        category = category_2.as_hamster()
        category.name = category_1.name
        with pytest.raises(ValueError):
            alchemy_store.categories._update(category)

    def test_remove(self, alchemy_category, alchemy_store):
        """Make sure passing a valid category removes it from the db."""
        assert alchemy_store.session.query(AlchemyCategory).get(
            alchemy_category.pk) is not None
        result = alchemy_store.categories.remove(alchemy_category.as_hamster())
        assert result is None
        assert alchemy_store.session.query(AlchemyCategory).get(
            alchemy_category.pk) is None
    ...
    def test_get_existing_pk(self, alchemy_category, alchemy_store):
        """Make sure method retrieves corresponding object."""
        result = alchemy_store.categories.get(alchemy_category.pk)
        assert result == alchemy_category

    def test_get_non_existing_pk(self, alchemy_store, alchemy_category):
        """Make sure we throw an error if PK can not be resolved."""
        with pytest.raises(KeyError):
            alchemy_store.categories.get(alchemy_category.pk + 1)

    def test_get_by_name(self, alchemy_category, alchemy_store):
        """Make sure a category can be retrieved by name."""
        result = alchemy_store.categories.get_by_name(alchemy_category.name)
        assert result == alchemy_category
    ...
    def test_get_or_create_get(self, alchemy_store, alchemy_category):
...
#    #        assert getattr(existing_activity, key) == value
    ...
    def test_add_new_with_alchemy_category(self, alchemy_store, activity, alchemy_category):
    ...
    def test_add_new_with_existing_name_and_category(self, alchemy_store, activity,
        ...
        activity.category.name = alchemy_activity.category.name
        old_activity_count = alchemy_store.session.query(AlchemyActivity).count()
        old_category_count = alchemy_store.session.query(AlchemyCategory).count()
        with pytest.raises(ValueError):
    ...
    def test_add_with_pk(self, alchemy_store, alchemy_activity):
    ...
    def test_update_with_existing_category(self, alchemy_store, alchemy_activity,
        ...
        activity.category = alchemy_category_factory().as_hamster()
        old_activity_count = alchemy_store.session.query(AlchemyActivity).count()
        old_category_count = alchemy_store.session.query(AlchemyCategory).count()
        alchemy_store.activities._update(activity)
    ...
    def test_remove_existing(self, alchemy_activity, alchemy_store):
        """Make sure removing an existsing activity works as intended."""
        count_before = alchemy_store.session.query(AlchemyActivity).count()
        assert alchemy_store.session.query(AlchemyActivity).get(
            alchemy_activity.pk) is not None
        result = alchemy_store.activities.remove(alchemy_activity)
        count_after = alchemy_store.session.query(AlchemyActivity).count()
        assert count_after < count_before
        assert result is True
        assert alchemy_store.session.query(AlchemyActivity).get(
            alchemy_activity.pk) is None
    ...
    def test_get_by_composite_valid(self, alchemy_store, alchemy_activity, raw):
        ...
        if raw:
            ...
            result is alchemy_activity
        else:
            assert result == activity
            result is not activity

    def test_get_by_composite_invalid_category(self, alchemy_store, alchemy_activity, category):
